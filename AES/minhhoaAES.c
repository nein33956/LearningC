...
unsigned int RotWord(unsigned int w){
    unsigned int byte1 = (w >> 24) & 0xFF; //dịch 24 bit lấy 1 byte
...
unsigned int XorRcon(unsigned int w, int j){     
    int Rc[] = { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a,
...
unsigned int NhanCot(unsigned int w){
    ...
    unsigned kq1 = Nhan2(byte1) ^ Nhan3(byte2) ^ byte3 ^ byte4;
    unsigned kq2 = byte1 ^ Nhan2(byte2) ^ Nhan3(byte3) ^ byte4;
    unsigned kq3 = byte1 ^ byte2 ^ Nhan2(byte3) ^ Nhan3(byte4);
    unsigned kq4 = Nhan3(byte1) ^ byte2 ^ byte3 ^ Nhan2(byte4);
...
unsigned int* GiaimaAES(unsigned int C[4], unsigned int key[4]){
	unsigned int *w = KeyExpansion(key);
	unsigned int *add = AddRoundKey(C, &w[40]);  
	printf("\nAdRoundKey: \n");
	ShowMatrix(add);
	unsigned int *InvShRows = InvShiftRows(add);
	printf("\nInvShiftRows: \n");
...
int main(){
	...
	//Bat dau test
	unsigned int rot = RotWord(w3); //cf4f3c09
	unsigned int sub = SubWord(rot); //8a84eb01
	unsigned int g3 = G(w3, 1); //8b84eb01
	unsigned int *w = KeyExpansion(Key);
	state = AddRoundKey(state, Key); //in ra chuyen vi
	state = SubBytes(state); //in ra chuyen vi
	state = ShiftRows(state);  //in ra chuyen vi
	state = MixColumns(state); //in ra chuyen vi
